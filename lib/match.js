...
export function matchLeadToBuyers(lead, buyers = []) {
  // Public buyer submissions remain `new` until explicitly approved.
  // Never route customer data or commercial opportunities to an unapproved buyer.
  const statusPool = buyers.filter((b) => b.status === 'approved');
  return statusPool
