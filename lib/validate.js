...
// Conservative pilot floors. These are qualification gates, not prices or buyer commitments.
export const SERVICE_RULES = {
  flooring: { minBudget: 2500 },
  'business-funding': { minBudget: 5000 },
  'mortgage-heloc': { minBudget: 5000 },
  'commercial-landscaping': { minBudget: 2500 },
  insulation: { minBudget: 1500 },
  construction: { minBudget: 2500 },
  handyman: { minBudget: 250 },
  'home-inspection': { minBudget: 300 },
  'home-estimator': { minBudget: 0 },
  probate: { minBudget: 0 },
  hvac: { minBudget: 1000 },
  roofing: { minBudget: 2000 },
  'home-security': { minBudget: 250 },
  marketing: { minBudget: 1000 },
  flowers: { minBudget: 100 },
  'dry-cleaning': { minBudget: 50 }
};
...
export function normalizeLead(body = {}) {
  ...
  return {
    ...
    service, service_slug: normalizeService(body.service_slug || service),
...
export function scoreLead(input = {}) {
  ...
  const rule = SERVICE_RULES[l.service_slug];
  if (budget !== null && rule) {
    if (budget >= Math.max(rule.minBudget * 3, 8000)) score += 18;
    else if (budget >= Math.max(rule.minBudget * 1.5, 1000)) score += 14;
    else if (budget >= rule.minBudget) score += 10;
  }
...
export function qualifyLead(input = {}, buyerPool = []) {
  ...
  lead.service_slug = normalizeService(lead.service_slug || lead.service);
  ...
  const rule = SERVICE_RULES[lead.service_slug];
  ...
  else if (!rule) disqualify_reason = 'unsupported_service';
  else if (lead.budget_cad !== null && lead.budget_cad < rule.minBudget) disqualify_reason = 'budget_below_service_minimum';
  if (!disqualify_reason && buyerPool.length && lead.budget_cad !== null) {
    const mins = buyerPool.filter((b) => b.service_slug === lead.service_slug && ['approved','new'].includes(b.status)).map((b) => Number(b.min_budget_cad || 0)).filter(Number.isFinite);
    if (mins.length && lead.budget_cad < Math.min(...mins)) disqualify_reason = 'budget_below_buyer_minimum';
  }
  const qualified = !disqualify_reason && lead.inGta && score >= 70;
